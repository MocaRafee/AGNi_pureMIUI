...
then
    make defconfig O=/mnt/ANDROID/COMPILED_OUT ARCH=arm64 agni_tulip-miui-pie_defconfig
fi

make -j3 O=/mnt/ANDROID/COMPILED_OUT ARCH=arm64

rm -rf $KERNELDIR/BUILT_tulip-miui-pie
mkdir -p $KERNELDIR/BUILT_tulip-miui-pie

#find -name '*.ko' -exec mv -v {} $KERNELDIR/BUILT_tulip-miui-pie/system/lib/modules \;

mv /mnt/ANDROID/COMPILED_OUT/arch/arm64/boot/Image.*-dtb $KERNELDIR/BUILT_tulip-miui-pie/
